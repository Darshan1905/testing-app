...
class CoverTypeWidget extends StatelessWidget {
  final GetMyPolicyBloc? gmpBloc;
  final String type;

  const CoverTypeWidget({Key? key, required this.gmpBloc, required this.type})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
        stream: type == "OVHC"
            ? gmpBloc?.getCoverTypeListStream
            : gmpBloc?.oSHCCoverTypeStream,
        builder: (context, snapshot) {
          ...
          return Container(
            ...
            child: DropdownButtonHideUnderline(
              child: StreamBuilder(
                  stream: gmpBloc?.getSelectedCoverTypeListStream,
                  builder: (context, selectedCoverType) {
                    return DropdownButton<PolicyData>(
                      ...
                      onChanged: (PolicyData? value) {
                        gmpBloc?.selectedCoverTypeStream.add(value!);
                      },
                    );
                  }),
            ),
