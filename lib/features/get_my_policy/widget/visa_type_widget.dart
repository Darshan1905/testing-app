...
class VisaTypeWidget extends StatelessWidget {
  final GetMyPolicyBloc? gmpBloc;

  const VisaTypeWidget({Key? key, required this.gmpBloc}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return StreamBuilder(
        stream: gmpBloc?.getVisaTypeListStream,
        builder: (context, snapshot) {
          ...
          return Container(
            ...
            child: DropdownButtonHideUnderline(
              ...
              child: StreamBuilder(
                  stream: gmpBloc?.getSelectedVisaTypeListStream,
                  builder: (context, selectedVisaType) {
                    return DropdownButton<PolicyData>(
                      value: selectedVisaType.hasData
                      ...
                      onChanged: (PolicyData? value) {
                        gmpBloc?.selectedVisaTypeStream.add(value!);
                        gmpBloc?.filterProvider(value!.id ?? 0);
                      },
                    );
                  }),
            ),
